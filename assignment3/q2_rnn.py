...
from q3_lstm_cell import LSTMCell
...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        mask = [True for word in sentence]
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings_reverse= tf.reverse(embeddings, [1])

        ### END YOUR CODE
        return embeddings, embeddings_reverse
    ...
    def add_prediction_op(self):
        ...
        if self.config.cell == "rnn":
            ...
            preds = self._bilstm_pred_op(x_, x_reverse_, cell_forward, cell_backwards)
        ...
        assert preds.get_shape().as_list() == [None, self.max_length, Config.n_classes], "predictions are not of the right shape. Expected {}, got {}".format([None, self.max_length, self.config.n_classes], preds.get_shape().as_list())
        return preds
    def _rnn_gru_pred_op(self, inputs, cell):
        preds = []
        with tf.variable_scope("pred", initializer = tf.contrib.layers.xavier_initializer()):
                ...
                # where tf.shape(x)[1] is the batch_size
                hidden_t = tf.zeros(shape = (tf.shape(inputs)[1], Config.hidden_size), dtype = tf.float32, name = "h")
                # cell_t = tf.zeros(shape = (tf.shape(x_)[1], Config.hidden_size), dtype = tf.float32, name = "cell_state")
                cell_t = tf.random_normal(shape = (tf.shape(inputs)[1], Config.hidden_size), stddev=0.1)
        ...
        with tf.variable_scope(self.config.cell): 
            for time_step in range(self.max_length): 
                ...
                output_t, hidden_t = cell(inputs[time_step], hidden_t)
                output_drop_t = tf.nn.dropout(output_t, self.dropout_placeholder)
                y_t = tf.matmul(output_drop_t, U) + b2
                preds.append(y_t)

        preds = tf.transpose(preds, [1, 0, 2]) 
        return(preds)

    def _lstm_pred_op(self, inputs, cell):
        preds = []
        ...
        with tf.variable_scope(self.config.cell): 
            for time_step in range(self.max_length): 
                ...
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables()
                output_t, cell_t, hidden_t = cell(inputs[time_step], cell_t, hidden_t)
                output_drop_t = tf.nn.dropout(output_t, self.dropout_placeholder)
                y_t = tf.matmul(output_drop_t, U) + b2
                preds.append(y_t)
    ...
    def _tf_lstm_pred_op(self, inputs, cell):
        preds = []

        with tf.variable_scope("pred", initializer = tf.contrib.layers.xavier_initializer()):
                U = tf.get_variable("U", shape = (Config.hidden_size, Config.n_classes))
                b2 = tf.get_variable("b2", shape = (Config.n_classes))
                # where tf.shape(inputs)[1] is the batch_size
                hidden_t = tf.zeros(shape = (tf.shape(inputs)[1], Config.hidden_size), dtype = tf.float32, name = "h")
                # cell_t = tf.zeros(shape = (tf.shape(x_)[1], Config.hidden_size), dtype = tf.float32, name = "cell_state")
                cell_t = tf.random_normal(shape = (tf.shape(inputs)[1], Config.hidden_size), stddev=0.1)
        with tf.variable_scope(self.config.cell):
        ...
        preds = tf.transpose(preds, [1, 0, 2])
        return(preds)
    def _bilstm_pred_op(self, inputs_forward, inputs_reverse, cell_forward, \
                        cell_reverse):
        preds = [] 
        with tf.variable_scope("pred", initializer = tf.contrib.layers.xavier_initializer()):
                U = tf.get_variable("U", shape = (Config.hidden_size * 2, Config.n_classes))
                b2 = tf.get_variable("b2", shape = (Config.n_classes))
                # where tf.shape(x)[1] is the batch_size
                hidden_t_forward = tf.zeros(shape = (tf.shape(inputs_forward)[1], Config.hidden_size), dtype = tf.float32)
                cell_t_forward = tf.random_normal(shape = (tf.shape(inputs_forward)[1], Config.hidden_size), stddev=0.1)

                hidden_t_reverse = tf.zeros(shape = (tf.shape(inputs_forward)[1], Config.hidden_size), dtype = tf.float32)
                cell_t_reverse = tf.random_normal(shape = (tf.shape(inputs_forward)[1], Config.hidden_size), stddev=0.1)

        with tf.variable_scope(self.config.cell):
            for time_step in range(self.max_length):
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables()
                output_t_forward, (cell_t_forward, hidden_t_forward) = cell_forward(inputs_forward[time_step], (cell_t_forward, hidden_t_forward), scope = "forward")
                output_t_reverse, (cell_t_reverse, hidden_t_reverse) = cell_reverse(inputs_reverse[time_step], (cell_t_reverse, hidden_t_reverse), scope = "backward")
                output_t_forward = tf.nn.dropout(output_t_forward, self.dropout_placeholder)
                output_t_reverse = tf.nn.dropout(output_t_reverse, self.dropout_placeholder)
                print(output_t_forward.get_shape()) 
                print(output_t_reverse.get_shape())
                output_drop_t = tf.concat((output_t_forward, output_t_reverse), axis = 1)
                print(output_drop_t.get_shape)
    ...
    def add_loss_op(self, preds):
        ...
        print("prediction shape: {}".format(preds.get_shape()))
        print("labels shape: {}".format(self.labels_placeholder.get_shape()))
        total_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels = self.labels_placeholder, logits = preds)
        print("total_loss_shape: {}".format(total_loss.get_shape()))
        print("mask_placeholder shape: {}".format(self.mask_placeholder))
        bool_masked_total_loss = tf.boolean_mask(total_loss, self.mask_placeholder)
        loss = tf.reduce_mean(bool_masked_total_loss)
        ### END YOUR CODE
        return loss
    ...
    def add_training_op(self, loss):
        ...
        # 1. No control over how the gradient is to be applied
        # train_op = tf.train.AdamOptimizer(Config.lr).minimize(loss)

        # 2. Use optimizer.compute_gradients to manually update gradients
        optimizer = tf.train.AdamOptimizer(Config.lr)

        # There are two ways to get the variables and the gradients
        # Method 1:
        # grads_and_vars = optimizer.compute_gradients(loss) # returns a list of (gradient, variable) pairs 
        # grad = [gv[0] for gv in grads_and_vars] # create a list of gradients
        # var = [gv[1] for gv in grads_and_vars] # create a list of variables 

        # Method 2:
        var = tf.trainable_variables() # returns the list of trainable tensors
        grad = tf.gradients(loss, var) # computes the loss with the trainable tensors

        # UNCOMMENT TO CLIP THE GRADIENT
...
if __name__ == "__main__":
    ...
    choices = ["rnn", "gru", "lstm", "bilstm", "tf-lstm", "tf-basiclstm"]
    ...
    command_parser.add_argument('-c', '--cell', choices=choices, default="rnn", help="Type of RNN cell to use.")
