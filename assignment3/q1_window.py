...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for i in range(window_size, len(sentence) - window_size):
            window_data = ([word for words in sentence[i - window_size: window_size + i + 1] for word in words],\
                           labels[i - window_size])
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(embed, [-1, Config.n_window_features * Config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name = "W", shape = [self.config.n_window_features * self.config.embed_size, \
                                                self.config.hidden_size], \
                                                initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable(name = "b1", shape = [self.config.hidden_size], initializer = tf.constant_initializer(0.0))
