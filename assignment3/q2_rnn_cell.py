...
class RNNCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope, initializer = tf.contrib.layers.xavier_initializer()):
            ...
            W_x = tf.get_variable("W_x", shape = (self.input_size, self.output_size))
            W_h = tf.get_variable("W_h", shape = (self.state_size, self.output_size))
            b = tf.get_variable("b", shape = (self.output_size))
            # print(inputs.get_shape())
            # print(W_x.get_shape())
            # print(state.get_shape())
            # print(W_h.get_shape())
            inputs_signal = tf.matmul(inputs, W_x) # inputs: batch_size * n_features (input_size)
            hidden_signal = tf.matmul(state, W_h) # state: batch_size * hidden_state_size
            h_t = inputs_signal + hidden_signal + b
            new_state = tf.sigmoid(h_t)
            ### END YOUR CODE ###
